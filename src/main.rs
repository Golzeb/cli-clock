...
fn generate_digit_art(number: i8, font: usize) -> Vec<String> {
    ...
    let number_art = CHARACTERS[font]
        .get((number % 10).try_into().unwrap_or(0usize))
        .unwrap();
...
fn print_time_art(time: &Time, center: bool, font: usize) -> () {
    ...
    out = out
        .iter()
        .zip(CHARACTERS[font].get(10).unwrap().iter())
        .map(|(x, y)| format!("{} {}", x, y))
        .collect();
    ...
    if center == true {
        ...
        final_out += "\n"
            .repeat(
                (term_size.rows / 2 - u16::try_from(out.len() / 2).unwrap())
                    .try_into()
                    .unwrap(),
            )
            .as_str();
        ...
        for s in out {
            final_out += format!(
                "{:>x$}\n",
                s,
                x = (term_size.cols / 2 + u16::try_from(length / 2).unwrap())
                    .try_into()
                    .unwrap()
            )
            .as_str();
